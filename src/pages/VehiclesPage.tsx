...
export function VehiclesPage() {
  ...
  const makePrimary = async (vehicleId: string) => {
    setBusy(true);
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Could not update vehicle.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const remove = async (vehicleId: string) => {
    if (!window.confirm('Remove this vehicle from WashRadar?')) return;
    setBusy(true);
    try {
  ...
  return <section className="community-page">
    <div className="community-hero">
      <div>
        <p className="eyebrow">MY CARS</p>
        <h1>Your garage, your wash preferences.</h1>
        <p>Save your vehicles now. WashRadar can use this foundation for vehicle-specific wash compatibility and recommendations without making your vehicle list public.</p>
      </div>
      <button className="primary-button" onClick={() => setShowForm((value) => !value)}><Plus size={17} /> Add vehicle</button>
    </div>

    {showForm && <form className="panel vehicle-form" onSubmit={submit}>
      <div className="section-heading">
        <div><p className="eyebrow">NEW VEHICLE</p><h2>Add a car</h2></div>
        <ShieldCheck size={22} />
      </div>
      ...
      <div className="form-actions">
        ...
        <button className="primary-button" disabled={busy}>{busy ? 'Adding…' : 'Add vehicle'}</button>
      </div>
    </form>}

    {!vehicles.length && !showForm ? <div className="panel empty-garage">
      <CarFront size={42} />
      <h2>No vehicles yet</h2>
      <p>Add your car so WashRadar can grow into vehicle-aware wash recommendations.</p>
      <button className="secondary-button" onClick={() => setShowForm(true)}>Add your first car</button>
      ...
      {vehicles.map((vehicle) => <article className={'panel vehicle-card ' + (vehicle.isPrimary ? 'primary-vehicle' : '')} key={vehicle.id}>
        ...
        <div className="vehicle-copy">
          <p className="eyebrow">{vehicle.isPrimary ? 'PRIMARY VEHICLE' : 'VEHICLE'}</p>
          <h2>{vehicle.nickname || `${vehicle.year} ${vehicle.make} ${vehicle.model}`}</h2>
          {vehicle.nickname && <p>{vehicle.year} {vehicle.make} {vehicle.model}</p>}
          {vehicle.vin && <small>VIN ending {vehicle.vin.slice(-4)}</small>}
        </div>
        <div className="vehicle-actions">
          {!vehicle.isPrimary && <button className="secondary-button" disabled={busy} onClick={() => void makePrimary(vehicle.id)}><Check size={15} /> Make primary</button>}
          <button className="icon-danger" disabled={busy} aria-label="Remove vehicle" onClick={() => void remove(vehicle.id)}><Trash2 size={17} /></button>
        </div>
      </article>)}
    </div>}
  </section>;

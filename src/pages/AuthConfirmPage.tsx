...
export function AuthConfirmPage() {
  ...
  if (mode === 'recovery') return <section className="auth-confirm-page"><div className="panel auth-confirm-card recovery"><div className="auth-confirm-icon"><KeyRound /></div><p className="eyebrow">PASSWORD RECOVERY</p><h1>Choose your WashRadar password.</h1><p>Use at least 8 characters. After this, normal sign-ins do not send an email.</p><form className="account-auth-form" onSubmit={savePassword}><label>New password<div className="auth-input-wrap"><KeyRound size={17} /><input type="password" required minLength={8} autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="At least 8 characters" /></div></label><label>Confirm new password<div className="auth-input-wrap"><KeyRound size={17} /><input type="password" required minLength={8} autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="Repeat password" /></div></label><button className="primary-button auth-primary" disabled={busy}>{busy ? 'Saving…' : 'Set password'}</button></form><p className="auth-email-note"><ShieldCheck size={16} /> Your password is handled by Supabase Auth and is never stored in WashRadar profile tables.</p></div></section>;

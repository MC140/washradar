...
-- The multi-ad selector receives a server-generated session hash and is intentionally
-- callable only by the service role through the ad-events Edge Function.
...
-- Keep the legacy one-ad RPC for older cached clients, but make its geography rule
-- radius-only as well. New clients use the server-side select path above.
